...
class PayslipDetailsReport(models.AbstractModel):
    _name = "report.ba_payroll.report_payslipdetails"
    _description = "Payslip Details Report"

    def get_details_by_rule_category(self, payslip_lines):
        PayslipLine = self.env["ba.hr.payslip.line"]
        RuleCateg = self.env["ba.hr.salary.rule.category"]
    ...
    def _get_report_values(self, docids, data=None):
        payslips = self.env["ba.hr.payslip"].browse(docids)
        return {
            "doc_ids": docids,
            "doc_model": "ba.hr.payslip",
            "docs": payslips,
            "data": data,
