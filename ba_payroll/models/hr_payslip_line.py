...
class HrPayslipLine(models.Model):
    _name = "ba.hr.payslip.line"
    _inherit = "ba.hr.salary.rule"
    _description = "Payslip Line"
    _order = "contract_id, sequence"

    slip_id = fields.Many2one(
        "ba.hr.payslip", string="Pay Slip", required=True, ondelete="cascade"
    )
    date_from = fields.Date("Date From", related="slip_id.date_from", store=True)
    payslip_run_id = fields.Many2one(
        "ba.hr.payslip.run", related="slip_id.payslip_run_id", string="Payslip Batch"
    )
    child_ids = fields.One2many(
        "ba.hr.payslip.line", "parent_line_id", string="Child Payslip Lines"
    )
    ...
    salary_rule_id = fields.Many2one("ba.hr.salary.rule", string="Rule", required=True)
    employee_id = fields.Many2one("hr.employee", string="Employee", required=True)
    contract_id = fields.Many2one(
        "hr.contract", string="Contract", required=True, index=True
    )
    rate = fields.Float(string="Rate (%)", digits="BA Payroll Rate", default=100.0)
    amount = fields.Float(digits="BA Payroll")
    quantity = fields.Float(digits="BA Payroll", default=1.0)
    total = fields.Float(
    ...
    def _compute_allow_edit_payslip_lines(self):
        self.allow_edit_payslip_lines = (
            ...
            .sudo()
            .get_param("ba_payroll.allow_edit_payslip_lines")
        )
    ...
    def create(self, vals_list):
        for values in vals_list:
            if "employee_id" not in values or "contract_id" not in values:
                payslip = self.env["ba.hr.payslip"].browse(values.get("slip_id"))
                values["employee_id"] = (
