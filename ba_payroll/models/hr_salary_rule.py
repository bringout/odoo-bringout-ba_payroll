...
class HrSalaryRule(models.Model):
    ...
    category_id = fields.Many2one("ba.hr.salary.rule.category", string="Category")
    ...
    parent_rule_id = fields.Many2one(
        comodel_name="ba.hr.salary.rule", string="Parent Salary Rule", index=True
    )
...
# result = worked_days.WORK0 and worked_days.WORK0.number_of_days > 0""",  # noqa: E501
    ...
    amount_fix = fields.Float(string="Fixed Amount", digits="BA Payroll")
    amount_percentage = fields.Float(
        string="Percentage (%)",
        digits="BA Payroll Rate",
...
# result = contract.wage * 0.10""",  # noqa: E501
    ...
    child_ids = fields.One2many(
        "ba.hr.salary.rule", "parent_rule_id", string="Child Salary Rule", copy=True
    )
    register_id = fields.Many2one(
        "ba.hr.contribution.register",
        string="Contribution Register",
        help="Eventual third party involved in the salary payment of the employees.",
    )
    input_ids = fields.One2many("ba.hr.rule.input", "input_id", string="Inputs", copy=True)
    ...
    def _check_parent_rule_id(self):
        if self.env.registry[self._name]._name != "ba.hr.salary.rule":
            # HrPayslipLine inherits from "ba.hr.salary.rule" and we don't
    ...
    def _recursive_search_of_rules(self):
        ...
        """
        if not self:
            return self.env["ba.hr.salary.rule"]
        else:
    ...
    def _compute_require_code_and_category(self):
        require = (
            ...
            .sudo()
            .get_param("ba_payroll.require_code_and_category")
        )
