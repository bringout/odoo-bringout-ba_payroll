...
class HrPayslip(models.Model):
    ...
    refunded_id = fields.Many2one(
        "ba.hr.payslip", string="Refunded Payslip", readonly=True
    )
    ...
    def _compute_allow_cancel_payslips(self):
        self.allow_cancel_payslips = (
            ...
            .sudo()
            .get_param("ba_payroll.allow_cancel_payslips")
        )
    ...
    def _compute_prevent_compute_on_confirm(self):
        self.prevent_compute_on_confirm = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("ba_payroll.prevent_compute_on_confirm")
        )
    ...
    def refund_sheet(self):
        ...
        formview_ref = self.env.ref("ba_payroll.hr_payslip_view_form", False)
        treeview_ref = self.env.ref("ba_payroll.hr_payslip_view_tree", False)
        res = {
            "name": _("Refund Payslip"),
            "view_mode": "tree, form",
            "view_id": False,
            "res_model": "ba.hr.payslip",
    ...
    def _compute_leave_days(self, contract, day_from, day_to, leave_domain=None):
        ...
        """
        leaves_positive = (
            self.env["ir.config_parameter"].sudo().get_param("ba_payroll.leaves_positive")
        )
        leaves = {}
    ...
    def get_inputs(self, contracts, date_from, date_to):
        ...
        rule_ids = (
            self.env["ba.hr.payroll.structure"].browse(structure_ids).get_all_rules()
        )
        sorted_rule_ids = [id for id, sequence in sorted(rule_ids, key=lambda x: x[1])]
        payslip_inputs = (
            self.env["ba.hr.salary.rule"].browse(sorted_rule_ids).mapped("input_ids")
        )
    ...
    def _get_salary_rules(self):
        rule_obj = self.env["ba.hr.salary.rule"]
