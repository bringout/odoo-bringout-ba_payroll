...
class HrSalaryRuleCategory(models.Model):
    _name = "ba.hr.salary.rule.category"
    _description = "Salary Rule Category"

    name = fields.Char(required=True, translate=True)
    code = fields.Char()
    parent_id = fields.Many2one(
        "ba.hr.salary.rule.category",
        string="Parent",
        help="Linking a salary category to its parent is used only for the "
        "reporting purpose.",
    )
    children_ids = fields.One2many(
        "ba.hr.salary.rule.category", "parent_id", string="Children"
    )
    salary_rules_ids = fields.One2many(
        "ba.hr.salary.rule", "category_id", string="Salary Rule Categories"
    )
