...
class TestPayslipFlow(TestPayslipBase):
    ...
    def test_00_payslip_flow(self):
        ...
        payslip_input = self.env["ba.hr.payslip.input"].search(
        ...
        richard_payslip.with_context(
            ...
            department_id=False,
            section_id=False,
            active_ids=[self.ref("ba_payroll.hr_payslip_menu")],
            active_id=self.ref("ba_payroll.hr_payslip_menu"),
        ).compute_sheet()
        ...
        payslip_refund = self.env["ba.hr.payslip"].search(
            [
        ...
        payslip_run = self.env["ba.hr.payslip.run"].create(
            {
                "date_end": "2011-09-30",
        ...
        payslip_employee = self.env["ba.hr.payslip.employees"].create(
        ...
        self.env["ba.payslip.lines.contribution.register"].create(
            {"date_from": "2011-09-30", "date_to": "2011-09-01"}
        )

        # I print the payslip report
        data, data_format = self.env.ref(
            "ba_payroll.action_report_payslip"
        )._render_qweb_pdf(
            self.env.ref("ba_payroll.action_report_payslip"), res_ids=richard_payslip.ids
        )

        # I print the payslip details report
        data, data_format = self.env.ref(
            "ba_payroll.payslip_details_report"
        )._render_qweb_pdf(
            self.env.ref("ba_payroll.payslip_details_report"), res_ids=richard_payslip.ids
        )

        # I print the contribution register report
        context = {
            "model": "ba.hr.contribution.register",
